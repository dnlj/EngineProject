...
namespace Game::Terrain {
	...
	class Generator {
		private:
			...
			// Generation threads.
			std::thread threads[8]{}; // TODO: Probably want a cvar for this.
			std::atomic_flag layerGenThreadShouldExit{};
		...
		public:
			...
			~Generator() {
				layerGenThreadShouldExit.test_and_set();
			...
			void generateLayers() {
				...
				Engine::forEach(layers, [&]<class Layer>(Layer& layer) ENGINE_INLINE_REL {
					...
					if (!reqs.partitions.empty()) {
						if constexpr (false /* Single/main threaded for debugging. */) {
		...
		private:
			void layerCoordinatorThread() {
				//
				//
				// TODO: move the request queue handling from generateLayers to its own thread here.
				//
				//
			}

			void layerGenerateThread() {
				ENGINE_LOG2("On thread: {}", std::this_thread::get_id()._Get_underlying_id());
				...
				while (true) {
					...
					while ((activeLayerGenNextPartition < 0) && !layerGenThreadShouldExit.test()) {
						layerGenThreadWait.wait(lock);
					}

					// Exit thread.
					if (layerGenThreadShouldExit.test()) { return; }
