...
namespace Engine::ECS {
	...
	constexpr inline size_t MAX_COMPONENTS = 64;

	/** The maximum number of systems registrable. Ideally this would be exactly the number of systems used. */
	constexpr inline size_t MAX_SYSTEMS = 64;

	/**
	 * The type used to store tick numbers.
	 * 
	 * Tick numbers are sequentially increasing and do _NOT_ wrap.
	 * They are _NOT_ a sequence number. Not wrapping should be safe, even for
	 * long running servers, because even at a high tick rate like 128 it would
	 * take over a year to exhaust the tick space:
	 ...
	 */
	using Tick = uint32;
	constexpr inline Tick invalidTick = 0;
